...
uint32_t kernel_max(std::vector<uint32_t> &square, int levels, unsigned x, unsigned y)
{
	int w = sqrt(square.size()); //This should always return an integer
	uint32_t maximum = MAX;
	auto tKernel = [&](int x, int y) -> uint32_t { return square[y*w+x]; };
	auto subSet = [&](int x1, int y1, int x2, int y2) -> std::vector<uint32_t> 
	{
		std::vector<uint32_t> sub_kernel;
	...
	//Note that even if it includes pixels that aren't needed, pass a full 3x3 kernel
		...
		{ 
		 case 0:switch(y)
			{
			 case 0: maximum = vmax(tKernel(0,0), tKernel(0,1), tKernel(1,0)); 
				 break;
			 case 1: maximum = vmax(tKernel(1,1), tKernel(0,0), tKernel(0,2), tKernel(1,1)); 	
				 break;
			 case 2: maximum = vmax(tKernel(0,2), tKernel(0,1), tKernel(1,2)); 
				 break;
			}
			break;
		 case 1:switch(y)
			{
			 case 0: maximum = vmax(tKernel(1,0), tKernel(1,1), tKernel(0,0), tKernel(2,0)); 
				 break;
			 case 1: maximum = vmax(tKernel(1,0), tKernel(0,1), tKernel(1,1), tKernel(2,1), tKernel(1,2));
				 break;
			 case 2: maximum = vmax(tKernel(1,2), tKernel(1,1), tKernel(0,2), tKernel(2,2) );
				 break;
			}
			break;
		 case 2:switch(y)
			{
			 case 0: maximum = vmax(tKernel(1,0), tKernel(2,0), tKernel(2,1));
				 break;
			 case 1: maximum = vmax(tKernel(1,1), tKernel(2,1), tKernel(2,0), tKernel(2,2));
				 break;
			 case 2: maximum = vmax(tKernel(2,2), tKernel(1,2), tKernel(2,1));
				 break;
			}
			break;
		...
		switch(x)
		{
		 case 0:switch(y) 
			{
			 case 0: subKernel = subSet(0,0, 2,2);
				 maximum = vmax(kernel_max(subKernel, 1, 0, 0),
						tKernel(0,2), tKernel(1,1), tKernel(2,0));				 
				 break;
			 case 1: subKernel = subSet(0, 0, 2, 2);
				 maximum = vmax(kernel_max(subKernel, 1, 0, 1), tKernel(1,0), tKernel(1,2), 
						tKernel(2,1), tKernel(0,3));
				 break;
			 case 2: subKernel = subSet(0,1, 2,3);
				 max_minor = vmax(kernel_max(subKernel, 1, 0, 2), tKernel(1,1), tKernel(1,3), tKernel(2,2));
				 maximum = vmax(max_minor, tKernel(0,0), tKernel(0,4));
				 break;
			 case 3: subKernel = subSet(0,2, 2,4);
				 maximum = vmax(kernel_max(subKernel, 1, 0,1),
						tKernel(0,1), tKernel(1,2), tKernel(1,4), tKernel(2,3)); 
 				 break;
			 case 4: subKernel = subSet(0,2, 2,4);
				 maximum = vmax(kernel_max(subKernel, 1, 0,2), tKernel(0,2), tKernel(1,3), tKernel(2,4));
				 break;
			}
			break;
		 case 1:
			...
			{
			 case 0: subKernel = subSet(0,0, 2,2);
				 maximum = vmax(kernel_max(subKernel, 1, 0,1), tKernel(3,0), tKernel(0,1), 
						tKernel(2,1), tKernel(1,2));
				 break;
			 case 1: subKernel = subSet(0,0, 2,2);
				 max_minor = vmax(kernel_max(subKernel, 1, 1,1), tKernel(0,0), tKernel(2,0),
						  tKernel(0,2), tKernel(2,2));
				 maximum = vmax(max_minor, tKernel(3,1), tKernel(1,3));
				 break;
			 case 2: subKernel = subSet(0,1, 2,3);
				 max_minor = vmax(kernel_max(subKernel, 1, 1,1), tKernel(0,1), tKernel(2,1),
						  tKernel(0,3), tKernel(2,3));
				 maximum = vmax(max_minor, tKernel(3,2), tKernel(1, 4));
				 break;
			 case 3: subKernel = subSet(0,2, 2,4);
				 max_minor = vmax(kernel_max(subKernel, 1, 1,1), tKernel(0,2), tKernel(2,2),
						  tKernel(0,4), tKernel(2,4));
				 maximum = vmax(max_minor, tKernel(1,1), tKernel(3,2));
				 break;
			 case 4: subKernel = subSet(0,2, 2,4);
				 maximum = vmax(kernel_max(subKernel, 1, 1,2), tKernel(0,3), tKernel(1,2), 
						tKernel(2,3), tKernel(3,4));
				 break;
			}
			break;
		 case 2:
			...
			{
			 case 0: subKernel = subSet(1,0, 3,2);
				 max_minor = vmax(kernel_max(subKernel, 1, 1,0), tKernel(1,1), tKernel(3,1));
				 maximum = vmax(max_minor, tKernel(0,0), tKernel(4,0), tKernel(2,2));
				 break;
			 case 1: subKernel = subSet(1,0, 3,2);
				 max_minor = vmax(kernel_max(subKernel, 1, 1,1), tKernel(1,0), tKernel(3,0),
						  tKernel(1,2), tKernel(3,2));
				 maximum = vmax(max_minor, tKernel(0,1), tKernel(4,1), tKernel(2,3));
				 break;
			 case 2: subKernel = subSet(0,1, 2,3);
				 subKernel2 = subSet(2,1, 4,3);
				 max_minor = vmax(kernel_max(subKernel, 1, 1,1), kernel_max(subKernel, 1, 1,1));
				 maximum = vmax(max_minor, tKernel(2,0), tKernel(2,1), tKernel(2,3), tKernel(2,4));
				 break;
			 case 3: subKernel = subSet(0,2, 2,4);
				 subKernel2 = subSet(2,2, 4,4);
				 maximum = vmax(kernel_max(subKernel, 1, 1,1), kernel_max(subKernel2, 1, 1,1), 
						tKernel(2,1), tKernel(2,2), tKernel(2,4));
				 break;
			 case 4: subKernel = subSet(0,2, 2,4);
				 subKernel2 = subSet(2,2, 2,4);
				 maximum = vmax(kernel_max(subKernel, 1, 1,2), kernel_max(subKernel, 1, 1,2), 
						tKernel(2,2), tKernel(2,3));
				 break;
		 ...
		 case 3:
			...
			{
			 case 0: subKernel = subSet(2,0, 4,2);
				 maximum = vmax(kernel_max(subKernel, 1, 1, 0), tKernel(1,0), tKernel(2,1), 
						tKernel(4,1), tKernel(3,2));
				 break;
			 case 1: subKernel = subSet(1,0, 3,2);
				 subKernel2 = subSet(2,0, 4,2);
				 maximum = vmax(kernel_max(subKernel, 1, 1,1), kernel_max(subKernel2, 1, 2,1),
						tKernel(3,0), tKernel(3,2), tKernel(3,3)); 
				 break;
			 case 2: subKernel = subSet(1,1, 3,3);
				 subKernel2 = subSet(2,1, 4,3);
				 max_minor = vmax(kernel_max(subKernel, 1, 1,1), kernel_max(subKernel, 1, 2,1)); 
				 maximum = vmax(max_minor, tKernel(3,0), tKernel(3,1), tKernel(3,3), tKernel(3,4));
				 break;
			 case 3: subKernel = subSet(1,2, 3,4);
				 subKernel2 = subSet(2,2, 4,4);
				 maximum = vmax(kernel_max(subKernel, 1, 1,1), kernel_max(subKernel2, 1, 2,1), 
			 ...
			 case 4: subKernel = subSet(1,2, 3,4);
				 subKernel2 = subSet(2,2, 4,4);
				 maximum = vmax(kernel_max(subKernel, 1, 1,2), kernel_max(subKernel2, 1, 2,2),
						tKernel(3,2), tKernel(3,3));
				 break; 
			}
			break;
		 case 4:
			...
			{
			 case 0: subKernel = subSet(2,0, 4,2);
				 maximum = vmax(kernel_max(subKernel, 1, 1,0), tKernel(4,1), tKernel(4,2));
				 break;
			 case 1: subKernel = subSet(2,0, 4,2);
				 maximum = vmax(kernel_max(subKernel, 1, 1,1), tKernel(4,0), tKernel(4,2), tKernel(4,3));
				 break;
			 case 2: subKernel = subSet(2,1, 4,3);
				 maximum = vmax(kernel_max(subKernel, 1, 1,1), tKernel(4,0), tKernel(4,1),
						tKernel(4,3), tKernel(4,4));
				 break;
			 case 3: subKernel = subSet(2,2, 4,4);
				 maximum = vmax(kernel_max(subKernel, 1, 1,1), tKernel(4,1), tKernel(4,2), tKernel(4,4));
				 break;
			 case 4: subKernel = subSet(2,2, 4,4);
				 maximum = vmax(kernel_max(subKernel, 1, 1,2), tKernel(4,2), tKernel(4,3));
				 break;
			}
			break;
		} // End switch(x)
		...
		if(y<levels){
			if (y==0){ //Only look at first row if first block
			 if(count==0){
			   for(int x=0; x<w; x++){
			   	img_ker = img_k(x,y);
				out(x,y) = kernel_max(img_ker, levels, x%(2*levels+1), y%(2*levels+1));
			   }
			} // else do nothing
			}else{
			  for(int x=0; x<w; x++)
			  {
			    img_ker = img_k(x,y);
			    out(x,y) = kernel_max(img_ker, levels, x%(2*levels+1) ,y); 
			   }
			 }
		 }else if(y<end){
			if(y<end-levels){
